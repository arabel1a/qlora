...
# Upstream MoE LoRA layers -- imported only to DISCARD them from the registry so
# our Ascend-native subclasses (which skip the GPU/Triton injection) win the
# can_replace_layer match for FusedMoE modules.
from vllm.lora.layers.fused_moe import (
    FusedMoE3DWithLoRA as _UpstreamFusedMoE3DWithLoRA,
    FusedMoEWithLoRA as _UpstreamFusedMoEWithLoRA,
)
...
def refresh_all_lora_classes():
    ...
    # MoE LoRA: remove the GPU/Triton upstream matchers so only the Ascend
    # native subclasses replace FusedMoE layers (set uses object identity, so
    # the __name__ shim on our classes does not collide with the discard).
    vllm.lora.utils._all_lora_classes.discard(_UpstreamFusedMoEWithLoRA)
    vllm.lora.utils._all_lora_classes.discard(_UpstreamFusedMoE3DWithLoRA)
