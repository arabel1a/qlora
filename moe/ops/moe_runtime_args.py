...
def build_mlp_compute_input(
    *,
    ...
    # MoE LoRA (PR #10977 backport): the AllGather combine metadata carries the
    # per-(token,k) permutation produced by npu_moe_init_routing; it is the only
    # comm mode that exposes it, so it is None on other paths (LoRA asserts).
