...
def unquant_apply_mlp(
    ...
    # MoE LoRA (backport of PR #10977): inject the w13 delta into gate_up_out
    # before activation. Only when an adapter wraps this layer and the AllGather
    # comm method supplied routing metadata (expanded_row_idx). Lazy import keeps
    # the common (non-LoRA) MLP path free of any LoRA dependency.
    lora_routing = None
    if lora_context is not None:
        if expanded_row_idx is None or topk_ids is None:
            raise AssertionError(
                "MoE LoRA requires expanded_row_idx and topk_ids metadata, "
                "only available in AllGather communication mode."
            )
    ...
    # MoE LoRA: inject the w2 delta into the down-proj output, reusing the
    # per-row routing computed for w13. gate_up_out is the activation output
    # that fed the base down GMM (the lora_a input for w2).
