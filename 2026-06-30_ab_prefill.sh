...
ADAPTER=/home/russia_mmo/models/Qwen3-32B-lora
...
boot() {  # $1 = tag, server log path implied
  ...
  vllm serve $MODEL \
    ...
    > $Q/logs/ab_prefill/server_${TAG}.log 2>&1 &
  for i in $(seq 1 90); do
    curl -sf http://0.0.0.0:${PORT}/health >/dev/null 2>&1 && { echo "[health ok @ ${i}0s] $TAG"; return 0; }
    sleep 10
  done
  echo "[!] BOOT FAILED $TAG"; tail -20 $Q/logs/ab_prefill/server_${TAG}.log; return 1
}
...
run_eval() {  # $1 = tag
  ...
  python run_evalscope.py \
    --number 64 --warmup-num 8 --parallel 16 \
    ...
    --outputs-dir $Q/logs/ab_prefill/out_${TAG} \
    --rate -1 2>&1 | tee $Q/logs/ab_prefill/eval_${TAG}.log
}

signals() {  # $1 = tag -- pull systemic-cause signals from server log
  local L=$Q/logs/ab_prefill/server_$1.log
  echo "---- SIGNALS $1 ----"
